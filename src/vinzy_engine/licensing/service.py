...
class LicensingService:
    ...
    async def get_customer_by_payment_id(
        self, session: AsyncSession, payment_provider: str, payment_id: str,
    ) -> CustomerModel | None:
        """Find a customer by payment provider + payment ID (idempotency lookup)."""
        if not payment_id:
            return None
        result = await session.execute(
            select(CustomerModel).where(
                CustomerModel.payment_provider == payment_provider,
                CustomerModel.payment_id == payment_id,
            )
        )
        return result.scalar_one_or_none()
